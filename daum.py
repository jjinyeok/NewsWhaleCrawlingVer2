...
article_title = response.xpath('/html/body/div[1]/main/section/div/article/div[1]/h3/text()').extract()
...
article_reporter = ''.join(response.xpath('/html/body/div[1]/main/section/div/article/div[1]/div[1]/span[1]/text()').extract())
article_url = link
aritcle_media_name = ''.join(response.xpath('/html/body/div[1]/header/div/h1/a[2]/text()').extract())
article_media_url = ''.join(response.xpath('/html/body/div[1]/header/div/h1/a[2]/@href').extract())
article_media_image_src = '' # daum은 media image를 제공하지 않음
article_last_modified_date = response.xpath('/html/body/div[1]/main/section/div/article/div[1]/div[1]/span/span/text()').extract()
if len(article_last_modified_date) == 0:
    article_last_modified_date = ''
else:
    article_last_modified_date = article_last_modified_date[-1]
article_last_modified_date = ''.join(article_last_modified_date.split())
# 키워드 추출하기 함수
